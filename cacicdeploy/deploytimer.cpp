...
void deployTimer::onTimer()
{
    ...
    QJsonObject getConfigJson = CCacic::getJsonFromFile(this->cacicFolder + "/getConfig.json");
    QJsonObject modulos;
    if( !getConfigJson.isEmpty()
            && !getConfigJson["agentcomputer"].toObject().isEmpty()
            && !getConfigJson["agentcomputer"].toObject()["modulos"].toObject().isEmpty() ) {

        QJsonObject modulos = getConfigJson["agentcomputer"].toObject()["modulos"].toObject();

        if( modulos.contains("outros") && modulos["outros"].isArray() ) {
            QJsonArray outrosModulos = modulos["outros"].toArray();
