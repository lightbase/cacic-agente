...
cacicD::cacicD(int argc, char **argv) : QtService<QCoreApplication>(argc, argv, "CacicDaemon")
{
    try{
        ...
        logManager->addDestination(application()->applicationDirPath() + "/Logs/cacicLog.log","Cacic Daemon",QLogger::InfoLevel);
        logManager->addDestination(application()->applicationDirPath() + "/Logs/cacicLog.log","Cacic Daemon",QLogger::ErrorLevel);
    } catch (...){
